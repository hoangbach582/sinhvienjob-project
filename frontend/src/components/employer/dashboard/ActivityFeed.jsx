...
import { FileText, CheckCircle, XCircle, Bell, Briefcase, UserCheck, Clock } from 'lucide-react';
import { ActivityItemSkeleton } from './SkeletonCard';

// Map type → icon và màu
const ACTIVITY_CONFIG = {
  new_application:    { icon: FileText,    color: '#3B82F6', bg: '#EFF6FF', label: 'Hồ sơ mới' },
  job_approved:       { icon: CheckCircle, color: '#10B981', bg: '#ECFDF5', label: 'Tin được duyệt' },
  job_rejected:       { icon: XCircle,     color: '#EF4444', bg: '#FEF2F2', label: 'Tin bị từ chối' },
  application_hired:  { icon: UserCheck,   color: '#8B5CF6', bg: '#F5F3FF', label: 'Đã tuyển' },
  new_job_posted:     { icon: Briefcase,   color: '#F59E0B', bg: '#FFFBEB', label: 'Đăng tin mới' },
...
function EmptyActivity() {
  return (
    <div style={{
      ...
      <p style={{ fontSize: '13px', color: '#94A3B8', margin: 0 }}>Chưa có hoạt động nào gần đây</p>
    </div>
  );
}
