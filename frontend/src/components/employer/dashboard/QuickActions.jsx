...
function ActionButton({ icon: Icon, label, description, gradient, textColor, onClick, secondary }) {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#94A3B8' }}>{description}</p>
          </div>
        </div>
        <ChevronRight size={16} color={hovered ? '#10B981' : '#CBD5E1'}
          style={{ transition: 'transform 0.2s, color 0.2s', transform: hovered ? 'translateX(4px)' : 'none' }}
        />
      </button>
    );
  }
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
        ...
        <div style={{ textAlign: 'left' }}>
          <p style={{ margin: 0, fontSize: '15px', fontWeight: 700, color: '#fff', display: 'flex', alignItems: 'center', gap: '6px' }}>
            {label}
            <Sparkles size={13} color="rgba(255,255,255,0.7)" />
          </p>
          <p style={{ margin: '3px 0 0', fontSize: '12px', color: 'rgba(255,255,255,0.75)' }}>{description}</p>
        </div>
      </div>

      <ChevronRight size={20} color="rgba(255,255,255,0.7)"
        style={{ transition: 'transform 0.2s', transform: hovered ? 'translateX(4px)' : 'none', flexShrink: 0 }}
      />
    </button>
  );
}
