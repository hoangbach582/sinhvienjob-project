...
function Jobs() {
    ...
    const fetchIndustries = async () => {
      try {
        const response = await fetch(
          (import.meta.env.VITE_API_URL || 'https://sinhvienjob-project.onrender.com/api') + "/categories/industries",
        );
        if (response.ok) {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${(import.meta.env.VITE_API_URL || 'https://sinhvienjob-project.onrender.com/api')}/jobs?${params.toString()}`, { headers });
      if (response.ok) {
