...
const getNotificationMeta = (notification) => {
  ...
  if (status === 'approved') {
    return { icon: <ApprovedIcon />, bgColor: '#ECFDF5', iconColor: '#10B981', borderColor: '#A7F3D0', label: 'Đã duyệt', labelBg: '#D1FAE5' };
  }
  if (status === 'rejected') {
    return { icon: <RejectedIcon />, bgColor: '#FEF2F2', iconColor: '#EF4444', borderColor: '#FECACA', label: 'Từ chối', labelBg: '#FEE2E2' };
  }
  if (type.includes('JobApplied') || type.includes('Application')) {
    return { icon: <ApplicationIcon />, bgColor: '#EFF6FF', iconColor: '#3B82F6', borderColor: '#BFDBFE', label: 'Ứng tuyển', labelBg: '#DBEAFE' };
  }
  return { icon: <DefaultIcon />, bgColor: '#F3F4F6', iconColor: '#6B7280', borderColor: '#E5E7EB', label: 'Thông báo', labelBg: '#F3F4F6' };
};

const EmployerNotifications = () => {
  const { notifications, fetchNotifications, markAsRead, markAllAsRead, deleteNotification, loading, unreadCount } = useNotifications();
  ...
  const filterButtons = [
    ...
    { key: 'approved', label: 'Đã duyệt', count: notifications.filter(n => n.data?.status === 'approved').length },
    { key: 'rejected', label: 'Từ chối', count: notifications.filter(n => n.data?.status === 'rejected').length },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '24px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: '800', color: '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{
                ...
                <svg xmlns="http://www.w3.org/2000/svg" style={{ height: '22px', width: '22px', color: 'white' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
              </span>
              Thông báo
            </h1>
            <p style={{ color: '#64748B', marginTop: '6px', fontSize: '14px', marginLeft: '52px' }}>
        ...
        {/* Filter Tabs */}
        <div style={{ display: 'flex', gap: '8px', marginTop: '20px', marginLeft: '52px', flexWrap: 'wrap' }}>
          {filterButtons.map(btn => (
            <button
              ...
              style={{
                padding: '6px 14px',
                borderRadius: '8px',
                border: filter === btn.key ? '1px solid #10B981' : '1px solid #E2E8F0',
        ...
        {filteredNotifications.length > 0 ? (
            ...
            {filteredNotifications.map((notification) => {
              ...
              return (
                <div
                  ...
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = isUnread ? '#FAFFFE' : 'white'; }}
                  onClick={() => {
          ...
          <div style={{ padding: '80px 24px', textAlign: 'center' }}>
            <div style={{
              ...
              <svg xmlns="http://www.w3.org/2000/svg" style={{ height: '40px', width: '40px', color: '#10B981' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
